...
export const DEFAULT_METHODS: PaymentMethod[] = [
  ...
  },
  {
    ...
    // also earns the 7.5% invoice discount (see lib/payment-plan.ts).
    method: "crypto",
    label: "Bitcoin",
    destination: "1ETThUdJPQUAM5BhM8vs6JqCYVhZFyNzLn",
    instructions:
      "Send exactly {{amount}} worth of BTC to {{destination}} on the {{network}} network only — transfers on any other network can't be recovered. Your 7.5% crypto discount is already included in this amount. Once sent, tap \"I've paid\" with your transaction ID so we can confirm fast. Reference: {{orderNumber}}.",
    network: "Bitcoin (BTC)",
    qrImageUrl: null,
    enabled: true,
    sortOrder: 4,
  },
  {
    ...
    destination: "",
    instructions:
      "Send {{amount}} via PayID to {{destination}}. Include your invoice number {{orderNumber}} in the description.",
    network: null,
    qrImageUrl: null,
    enabled: false,
    sortOrder: 5,
