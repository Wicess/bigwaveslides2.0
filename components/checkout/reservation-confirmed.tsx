...
export function ReservationConfirmed({
  ...
  totalCents: number;
  plan: PaymentPlan;
  /** Raw number from Settings -> Contact. Omit/empty hides the WhatsApp CTA. */
  ...
  // null when Settings has no WhatsApp number — the CTA is skipped entirely.
  const waHref = whatsappLink(message, whatsapp);
      ...
      <div className="border-border overflow-hidden rounded-3xl border bg-white shadow-[var(--shadow-soft)]">
        ...
        <div className="space-y-5 p-6">
          {/* Primary CTA — WhatsApp brand green, not the site primary, because
              ...
              Rendered ONLY when a number is configured in Settings: a dead
              wa.me link on a confirmation screen is worse than no button,
              because the customer thinks they have reached someone. */}
          {waHref ? (
            <div>
              <a
                href={waHref}
