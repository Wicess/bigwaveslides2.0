...
export function WhatsAppFab({ phone, label, message }: WhatsAppFabProps) {
  ...
  return (
    ...
    <a
      ...
      aria-label={label}
      className="group fixed bottom-[calc(2.5rem+var(--explore-bar-h,0px))] left-3 z-50 flex items-center gap-1.5 rounded-full py-1.5 pr-3 pl-1.5 text-white shadow-[0_10px_30px_-8px_rgba(16,142,86,0.7)] ring-1 ring-white/15 transition-[transform,bottom] duration-200 [background:linear-gradient(135deg,#3ad07f_0%,#22b06a_45%,#109e5e_100%)] hover:scale-[1.04] sm:bottom-6 sm:left-6 sm:gap-2.5 sm:py-2.5 sm:pr-5 sm:pl-2.5"
    >
      ...
      <span className="flex size-6 items-center justify-center gap-[2px] rounded-full bg-white/20 shadow-inner sm:size-9 sm:gap-[3px]">
        <span className="animate-wa-dot-1 size-1 rounded-full bg-white sm:size-1.5" />
        <span className="animate-wa-dot-2 size-1 rounded-full bg-white sm:size-1.5" />
        <span className="animate-wa-dot-3 size-1 rounded-full bg-white sm:size-1.5" />
      </span>
      ...
      <span aria-hidden className="text-left leading-none">
        <span className="block text-[11px] font-extrabold tracking-wide uppercase sm:text-sm">
          Chat now
        </span>
